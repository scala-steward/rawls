...
trait RawlsGroupComponent {
  ...
  object rawlsGroupQuery extends TableQuery(new RawlsGroupTable(_)) {
    ...
    def load(groupRef: RawlsGroupRef): ReadAction[Option[RawlsGroup]] = {
      ...
      val groupID = s"group with name '$name'"
      loadGroup(findGroupByName(name), groupID)
    }

    def loadGroupByEmail(groupEmail: RawlsGroupEmail): ReadAction[Option[RawlsGroup]] = {
      val email = groupEmail.value
      val groupID = s"group with email '$email'"
      loadGroup(findGroupByEmail(email), groupID)
    }
      ...
      groupsToCheck.headOption match {
        case Some(checkingNow) =>
          ...
          load(checkingNow).flatMap {
            ...
            case Some(group) =>
              val newGroups = group.subGroups -- previouslyCheckedGroups
              checkMembershipRecursively(userRef, previouslyCheckedGroups + checkingNow, remainingToCheck ++ newGroups)
  ...
  private def loadGroup(query: GroupQuery, groupID: String): ReadAction[Option[RawlsGroup]] = {
