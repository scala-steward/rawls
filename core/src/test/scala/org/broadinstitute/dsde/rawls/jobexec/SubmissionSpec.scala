...
import bio.terra.workspace.model.{CloningInstructionsEnum, DataRepoSnapshot, DataRepoSnapshotAttributes, ReferenceTypeEnum}
...
class SubmissionSpec(_system: ActorSystem) extends TestKit(_system)
  ...
  def dataRepoSubmissionTest[T](tableData: Map[String, String])(test: (WorkspaceService, MethodConfiguration, UUID) => T) = {
    ...
    withDataAndService({ workspaceService =>
      workspaceService.workspaceManagerDAO.createDataRepoSnapshotReference(minimalTestData.workspace.workspaceIdAsUUID, dataReferenceName, dataReferenceDescription, ReferenceTypeEnum.DATA_REPO_SNAPSHOT, new DataRepoSnapshotAttributes().instanceName(dataRepoDAO.getInstanceName).snapshot(snapshotUUID.toString), CloningInstructionsEnum.NOTHING, userInfo.accessToken)
