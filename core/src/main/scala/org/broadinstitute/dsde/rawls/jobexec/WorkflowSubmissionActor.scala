...
trait WorkflowSubmission extends FutureSupport with LazyLogging with MethodWiths with RawlsInstrumented {
  ...
  def submitWorkflowBatch(batch: WorkflowBatch)(implicit executionContext: ExecutionContext): Future[WorkflowSubmissionMessage] = {
    ...
    val cromwellSubmission = for {
      ...
      drsClientEmails <- resolveDrsUriServiceAccounts(dosUris, petUserInfo)
      ...
      _ <- if (drsClientEmails.isEmpty) Future.successful(false) else googleServicesDAO.addPolicyBindings(RawlsBillingProjectName(wfOpts.google_project), Map(requesterPaysRole -> drsClientEmails.map("serviceAccount:"+_)))
