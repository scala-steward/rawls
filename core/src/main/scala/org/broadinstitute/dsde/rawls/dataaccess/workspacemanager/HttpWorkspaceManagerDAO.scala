...
class HttpWorkspaceManagerDAO(baseWorkspaceManagerUrl: String)(implicit val system: ActorSystem, val materializer: Materializer, val executionContext: ExecutionContext) extends WorkspaceManagerDAO {
  ...
  override def createDataRepoSnapshotReference(workspaceId: UUID, name: DataReferenceName, description: Option[DataReferenceDescriptionField], referenceType: ReferenceTypeEnum, attributes: DataRepoSnapshotAttributes, cloningInstructions: CloningInstructionsEnum, accessToken: OAuth2BearerToken): DataRepoSnapshotResource = {
    val resourceMetadata = new ReferenceResourceCommonFields().name(name.value).cloningInstructions(cloningInstructions)
    description.map(d => resourceMetadata.description(d.value))
    val request = new CreateDataRepoSnapshotReferenceRequestBody().snapshot(attributes).metadata(resourceMetadata)
