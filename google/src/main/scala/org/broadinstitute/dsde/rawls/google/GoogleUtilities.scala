...
trait GoogleUtilities extends LazyLogging with Retry {
  ...
  protected def executeGoogleCall[A,B](request: AbstractGoogleClientRequest[A])(processResponse: (com.google.api.client.http.HttpResponse) => B)(implicit counters: GoogleCounters[A], retryCount: Int = 0): B = {
    ...
    Try {
      ...
      case Success(response) =>
        ...
        instrumentGoogleRequestResponse(request, start, Right(response), isRetry)
        try {
      ...
      case Failure(httpRegrets: HttpResponseException) =>
        ...
        instrumentGoogleRequestResponse(request, start, Left(httpRegrets), isRetry)
        throw httpRegrets
  ...
  private def instrumentGoogleRequestResponse[A](request: AbstractGoogleClientRequest[A], startTime: Long, responseOrException: Either[HttpResponseException, com.google.api.client.http.HttpResponse], isRetry: Boolean)(implicit counters: GoogleCounters[A]): Unit = {
    val c = counters(request, responseOrException, isRetry)
    c._1 += 1
    c._2 += 1
    c._3.update(System.currentTimeMillis() - startTime, TimeUnit.MILLISECONDS)
  }
