...
class ExpressionEvaluator(slickEvaluator: SlickExpressionEvaluator, val rootEntities: Option[Seq[EntityRecord]]) {
  ...
  The overall approach is:
    ...
    */
  def evalFinalAttribute(workspaceContext: Workspace, expression: String, inputOption: Option[MethodInput] = None)(implicit executionContext: ExecutionContext): ReadWriteAction[Map[EntityName, Try[Iterable[AttributeValue]]]] = {
      ...
      case Success(parsedTree) =>
        ...
        */
        ...
        localFinalAttributeEvaluationVisitor.visit(parsedTree).map { result =>
          InputExpressionReassembler.constructFinalInputValues(result, parsedTree, rootEntities.map(_.map(_.name)), inputOption)
        }
