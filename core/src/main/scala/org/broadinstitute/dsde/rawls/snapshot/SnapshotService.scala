...
class SnapshotService(protected val userInfo: UserInfo, val dataSource: SlickDataSource, val samDAO: SamDAO, workspaceManagerDAO: WorkspaceManagerDAO, bqServiceFactory: GoogleBigQueryServiceFactory, terraDataRepoInstanceName: String, pathToCredentialJson: String, clientEmail: WorkbenchEmail, deltaLayerStreamerEmail: WorkbenchEmail)
  ...
  def createSnapshot(workspaceName: WorkspaceName, snapshot: NamedDataRepoSnapshot): Future[DataRepoSnapshotResource] = {
    getWorkspaceContextAndPermissions(workspaceName, SamWorkspaceActions.write, Some(WorkspaceAttributeSpecs(all = false))).flatMap { workspaceContext =>
      ...
      val dataRepoAttributes = new DataRepoSnapshotAttributes().instanceName(terraDataRepoInstanceName).snapshot(snapshot.snapshotId)
      val snapshotRef = workspaceManagerDAO.createDataRepoSnapshotReference(workspaceContext.workspaceIdAsUUID, snapshot.name, snapshot.description, ReferenceTypeEnum.DATA_REPO_SNAPSHOT, dataRepoAttributes, CloningInstructionsEnum.NOTHING, userInfo.accessToken)
      ...
      createDatasetIO.unsafeToFuture().recover {
        case t: Throwable =>
          ...
          IO.pure(workspaceManagerDAO.deleteDataRepoSnapshotReference(workspaceContext.workspaceIdAsUUID, snapshotRef.getMetadata.getResourceId, userInfo.accessToken)).unsafeToFuture()
        ...
        createBQReferenceFuture.recover {
          case t: Throwable =>
            ...
            for {
              ...
              _ <- IO.pure(workspaceManagerDAO.deleteDataRepoSnapshotReference(workspaceContext.workspaceIdAsUUID, snapshotRef.getMetadata.getResourceId, userInfo.accessToken)).unsafeToFuture()
            } yield {}
            throw new RawlsExceptionWithErrorReport(ErrorReport(StatusCodes.InternalServerError, s"Unable to create snapshot reference in workspace ${workspaceContext.workspaceId}. Error: ${t.getMessage}"))
        }
      }.map { _ => snapshotRef}
