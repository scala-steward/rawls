...
class HttpGoogleServicesDAOSpec extends FlatSpec with Matchers with IntegrationTestConfig with Retry with TestDriverComponent with BeforeAndAfterAll with LazyLogging with Eventually with MockitoTestUtils with StatsDTestUtils {
  ...
  it should "do all of the things" in {
    ...
    val bucketResource = Await.result(retry(when500)(_ => Future { storage.buckets.get(googleWorkspaceInfo.bucketName).execute() }), Duration.Inf)
    ...
    val readerBAC = Await.result(retry(when500)(_ => Future { storage.bucketAccessControls.get(googleWorkspaceInfo.bucketName, gcsDAO.makeGroupEntityString(readerGroup.groupEmail.value)).execute() }), Duration.Inf)
    val writerBAC = Await.result(retry(when500)(_ => Future { storage.bucketAccessControls.get(googleWorkspaceInfo.bucketName, gcsDAO.makeGroupEntityString(writerGroup.groupEmail.value)).execute() }), Duration.Inf)
    val ownerBAC = Await.result(retry(when500)(_ => Future { storage.bucketAccessControls.get(googleWorkspaceInfo.bucketName, gcsDAO.makeGroupEntityString(ownerGroup.groupEmail.value)).execute() }), Duration.Inf)
    val projectOwnerBAC = Await.result(retry(when500)(_ => Future { storage.bucketAccessControls.get(googleWorkspaceInfo.bucketName, gcsDAO.makeGroupEntityString(projectOwnerGroup.groupEmail.value)).execute() }), Duration.Inf)
    val svcAcctBAC = Await.result(retry(when500)(_ => Future { storage.bucketAccessControls.get(googleWorkspaceInfo.bucketName, "user-" + gcsDAO.serviceAccountClientId).execute() }), Duration.Inf)
    ...
    // check that the access level for each group is what we expect
    val readerDOAC = Await.result(retry(when500)(_ => Future { storage.defaultObjectAccessControls.get(googleWorkspaceInfo.bucketName, gcsDAO.makeGroupEntityString(readerGroup.groupEmail.value)).execute() }), Duration.Inf)
    val writerDOAC = Await.result(retry(when500)(_ => Future { storage.defaultObjectAccessControls.get(googleWorkspaceInfo.bucketName, gcsDAO.makeGroupEntityString(writerGroup.groupEmail.value)).execute() }), Duration.Inf)
    val ownerDOAC = Await.result(retry(when500)(_ => Future { storage.defaultObjectAccessControls.get(googleWorkspaceInfo.bucketName, gcsDAO.makeGroupEntityString(ownerGroup.groupEmail.value)).execute() }), Duration.Inf)
    val projectOwnerDOAC = Await.result(retry(when500)(_ => Future { storage.defaultObjectAccessControls.get(googleWorkspaceInfo.bucketName, gcsDAO.makeGroupEntityString(projectOwnerGroup.groupEmail.value)).execute() }), Duration.Inf)
    val svcAcctDOAC = Await.result(retry(when500)(_ => Future { storage.defaultObjectAccessControls.get(googleWorkspaceInfo.bucketName, "user-" + gcsDAO.serviceAccountClientId).execute() }), Duration.Inf)
    ...
    Await.result(retry(when500)(_ => Future { directory.groups.get(readerGroup.groupEmail.value).execute() }), Duration.Inf)
    Await.result(retry(when500)(_ => Future { directory.groups.get(writerGroup.groupEmail.value).execute() }), Duration.Inf)
    Await.result(retry(when500)(_ => Future { directory.groups.get(ownerGroup.groupEmail.value).execute() }), Duration.Inf)
    Await.result(retry(when500)(_ => Future { directory.groups.get(projectOwnerGroup.groupEmail.value).execute() }), Duration.Inf)

    val ownerMembers = Await.result(retry(when500)(_ => Future { directory.members().list(ownerGroup.groupEmail.value).execute() }), Duration.Inf)
  ...
  it should "do all of the things with a realm" in {
    ...
    val readerIGBAC = Await.result(retry(when500)(_ => Future { storage.bucketAccessControls.get(googleWorkspaceInfo.bucketName, gcsDAO.makeGroupEntityString(readerIG.groupEmail.value)).execute() }), Duration.Inf)
    val writerIGBAC = Await.result(retry(when500)(_ => Future { storage.bucketAccessControls.get(googleWorkspaceInfo.bucketName, gcsDAO.makeGroupEntityString(writerIG.groupEmail.value)).execute() }), Duration.Inf)
    val ownerIGBAC = Await.result(retry(when500)(_ => Future { storage.bucketAccessControls.get(googleWorkspaceInfo.bucketName, gcsDAO.makeGroupEntityString(ownerIG.groupEmail.value)).execute() }), Duration.Inf)
    val projectOwnerIGBAC = Await.result(retry(when500)(_ => Future { storage.bucketAccessControls.get(googleWorkspaceInfo.bucketName, gcsDAO.makeGroupEntityString(projectOwnerIG.groupEmail.value)).execute() }), Duration.Inf)
    val svcAcctBAC = Await.result(retry(when500)(_ => Future { storage.bucketAccessControls.get(googleWorkspaceInfo.bucketName, "user-" + gcsDAO.serviceAccountClientId).execute() }), Duration.Inf)
    ...
    val readerIGDOAC = Await.result(retry(when500)(_ => Future { storage.defaultObjectAccessControls.get(googleWorkspaceInfo.bucketName, gcsDAO.makeGroupEntityString(readerIG.groupEmail.value)).execute() }), Duration.Inf)
    val writerIGDOAC = Await.result(retry(when500)(_ => Future { storage.defaultObjectAccessControls.get(googleWorkspaceInfo.bucketName, gcsDAO.makeGroupEntityString(writerIG.groupEmail.value)).execute() }), Duration.Inf)
    val ownerIGDOAC = Await.result(retry(when500)(_ => Future { storage.defaultObjectAccessControls.get(googleWorkspaceInfo.bucketName, gcsDAO.makeGroupEntityString(ownerIG.groupEmail.value)).execute() }), Duration.Inf)
    val projectOwnerIGDOAC = Await.result(retry(when500)(_ => Future { storage.defaultObjectAccessControls.get(googleWorkspaceInfo.bucketName, gcsDAO.makeGroupEntityString(projectOwnerIG.groupEmail.value)).execute() }), Duration.Inf)
    val svcAcctDOAC = Await.result(retry(when500)(_ => Future { storage.defaultObjectAccessControls.get(googleWorkspaceInfo.bucketName, "user-" + gcsDAO.serviceAccountClientId).execute() }), Duration.Inf)
    ...
    // check that the groups exist (i.e. that this doesn't throw exceptions)
    val directory = gcsDAO.getGroupDirectory
    Await.result(retry(when500)(_ => Future { directory.groups.get(readerAG.groupEmail.value).execute() }), Duration.Inf)
    Await.result(retry(when500)(_ => Future { directory.groups.get(writerAG.groupEmail.value).execute() }), Duration.Inf)
    Await.result(retry(when500)(_ => Future { directory.groups.get(ownerAG.groupEmail.value).execute() }), Duration.Inf)
    Await.result(retry(when500)(_ => Future { directory.groups.get(readerIG.groupEmail.value).execute() }), Duration.Inf)
    Await.result(retry(when500)(_ => Future { directory.groups.get(writerIG.groupEmail.value).execute() }), Duration.Inf)
    Await.result(retry(when500)(_ => Future { directory.groups.get(ownerIG.groupEmail.value).execute() }), Duration.Inf)
    Await.result(retry(when500)(_ => Future { directory.groups.get(projectOwnerIG.groupEmail.value).execute() }), Duration.Inf)

    //check that the owner is a part of both the access group and the intersection group
    Await.result(retry(when500)(_ => Future { directory.members.get(ownerAG.groupEmail.value, gcsDAO.toProxyFromUser(testCreator)).execute() }), Duration.Inf)
    Await.result(retry(when500)(_ => Future { directory.members.get(ownerIG.groupEmail.value, gcsDAO.toProxyFromUser(testCreator)).execute() }), Duration.Inf)
    Await.result(retry(when500)(_ => Future { directory.members.get(projectOwnerIG.groupEmail.value, gcsDAO.toProxyFromUser(testCreator)).execute() }), Duration.Inf)
  ...
  it should "create a project" in {
    ...
    try {
      ...
      val doneCreateOp = Await.result(retryUntilSuccessOrTimeout(always)(10 seconds, 1 minutes) { _ =>
      ...
      val doneServicesOps = Await.result(retryUntilSuccessOrTimeout(always)(10 seconds, 6 minutes) { _ =>
  ...
  it should "set lifecycle policy when deleting non-empty bucket" in {
    ...
    Await.result(retry(when500)(_ => Future { bucketInserter.execute() }), Duration.Inf)
     ...
     */
    val rule = Await.result(retryUntilSuccessOrTimeout(failureLogMessage = "Bucket has no lifecycle rules")(1 second, 10 seconds) { _ =>
  ...
  it should "determine bucket usage" in {
    ...
    try {
      ...
      try {
        for (i <- 1 to 2) {
          Await.result(retry(when500)(_ => Future {
  ...
  it should "return 'Not Available' when there are no storage logs and the bucket is NOT empty" in {
    ...
    val caught = intercept[GoogleStorageLogException] {
      try {
        ...
        Await.result(retry(when500)(_ => Future { storage.objects().delete(bucketName, "test-object").execute() }), Duration.Inf)
        Await.result(gcsDAO.deleteBucket(bucketName, bucketDeletionMonitor), Duration.Inf)
      }
    }
  ...
  private def insertObject(bucketName: String, objectName: String, content: String): String = {
    ...
    Await.result(retry(when500)(_ => Future { inserter.execute() }), Duration.Inf)
    objectName
  }
