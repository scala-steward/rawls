...
class SnapshotServiceSpec extends AnyWordSpecLike with Matchers with MockitoSugar with TestDriverComponent {
  ...
  "SnapshotService" should {
    "create a new snapshot reference to a TDR snapshot" in withMinimalTestDatabase { dataSource =>
      ...
      when(mockWorkspaceManagerDAO.createDataRepoSnapshotReference(any[UUID], any[DataReferenceName], any[Option[DataReferenceDescriptionField]], any[ReferenceTypeEnum], any[DataRepoSnapshotAttributes], any[CloningInstructionsEnum], any[OAuth2BearerToken])).thenReturn(new DataRepoSnapshotResource().attributes(new DataRepoSnapshotAttributes().snapshot(UUID.randomUUID().toString)).metadata(new ResourceMetadata().resourceId(UUID.randomUUID()).name("foo").description("").workspaceId(UUID.randomUUID()).cloningInstructions(CloningInstructionsEnum.NOTHING)))
      ...
      verify(mockWorkspaceManagerDAO, times(1)).createDataRepoSnapshotReference(any[UUID], any[DataReferenceName], any[Option[DataReferenceDescriptionField]], any[ReferenceTypeEnum], any[DataRepoSnapshotAttributes], any[CloningInstructionsEnum], any[OAuth2BearerToken])
    ...
    "not leave an orphaned bq dataset if creating a snapshot reference fails" in withMinimalTestDatabase { dataSource =>
      ...
      when(mockWorkspaceManagerDAO.createDataRepoSnapshotReference(any[UUID], any[DataReferenceName], any[Option[DataReferenceDescriptionField]], any[ReferenceTypeEnum], any[DataRepoSnapshotAttributes], any[CloningInstructionsEnum], any[OAuth2BearerToken])).thenThrow(new RuntimeException("oh no!"))
    ...
    "remove all resources when a snapshot reference is deleted" in withMinimalTestDatabase { dataSource =>
      ...
      when(mockWorkspaceManagerDAO.getDataRepoSnapshotReference(any[UUID], any[UUID], any[OAuth2BearerToken])).thenReturn(new DataRepoSnapshotResource().attributes(new DataRepoSnapshotAttributes().snapshot(UUID.randomUUID().toString)).metadata(new ResourceMetadata().resourceId(UUID.randomUUID()).name("foo").description("").workspaceId(UUID.randomUUID()).cloningInstructions(CloningInstructionsEnum.NOTHING)))

      when(mockWorkspaceManagerDAO.getBigQueryDatasetReferenceByName(any[UUID], any[String], any[OAuth2BearerToken])).thenReturn(new GcpBigQueryDatasetResource().metadata(new ResourceMetadata().resourceId(UUID.randomUUID())).attributes(new GcpBigQueryDatasetAttributes))
