...
class HttpGoogleServicesDAO(
  ...
  ) extends GoogleServicesDAO(groupsPrefix) with Retry with FutureSupport with LazyLogging with GoogleUtilities with GoogleInstrumented {
  ...
  override def deleteBucket(bucketName: String, monitorRef: ActorRef): Future[Unit] = {
    ...
    retryWithRecoverWhen500orGoogleError(count => {
      executeGoogleRequest(deleter)(Subsystems.GoogleBuckets, count)
  ...
  override def removeUserFromProxyGroup(user: RawlsUser): Future[Unit] = {
    ...
    retryWhen500orGoogleError(count => { executeGoogleRequest(deleter)(Subsystems.GoogleGroups, count) })
  }
  ...
  def deleteProxyGroup(user: RawlsUser): Future[Unit] = {
    ...
    retryWhen500orGoogleError (count => {
      ...
      executeGoogleRequest(deleter)(Subsystems.GoogleGroups, count)
    })
  }
  ...
  override def removeEmailFromGoogleGroup(groupEmail: String, emailToRemove: String): Future[Unit] = {
    ...
    retryWithRecoverWhen500orGoogleError[Unit](count => { executeGoogleRequest(deleter)(Subsystems.GoogleGroups, count) }) {
      ...
      credentialOpt match {
        ...
        case Some(credential) => {
          ...
          try {
            ...
            executeGoogleRequest(remover)(Subsystems.GoogleBuckets)
            None
          } catch {
  ...
  override def deleteToken(rawlsUserRef: RawlsUserRef): Future[Unit] = {
    retryWhen500orGoogleError(count => {
      executeGoogleRequest(getStorage(getBucketServiceAccountCredential).objects().delete(tokenBucketName, rawlsUserRef.userSubjectId.value))(Subsystems.GoogleBuckets, count)
    } )
  }
  ...
  override def getGoogleProject(projectName: RawlsBillingProjectName): Future[Project] = {
    ...
    retryWhen500orGoogleError(count => {
      executeGoogleRequest(cloudResManager.projects().get(projectName.value))(Subsystems.GoogleBilling, count)
    })
  }
  ...
  override def createProject(projectName: RawlsBillingProjectName, billingAccount: RawlsBillingAccount): Future[RawlsBillingProjectOperationRecord] = {
    ...
    retryWhen500orGoogleError(count => {
      executeGoogleRequest(cloudResManager.projects().create(new Project().setName(projectName.value).setProjectId(projectName.value).setLabels(Map("billingaccount" -> labelSafeString(billingAccount.displayName)))))(Subsystems.GoogleBilling, count)
    }).recover {
  ...
  override def pollOperation(rawlsBillingProjectOperation: RawlsBillingProjectOperationRecord): Future[RawlsBillingProjectOperationRecord] = {
    ...
    rawlsBillingProjectOperation.api match {
      case API_CLOUD_RESOURCE_MANAGER =>
        ...
        retryWhen500orGoogleError(count => {
          executeGoogleRequest(cloudResManager.operations().get(rawlsBillingProjectOperation.operationId))(Subsystems.GoogleBilling, count)
        }).map { op =>
      ...
      case API_SERVICE_MANAGEMENT =>
        ...
        retryWhen500orGoogleError(count => {
          executeGoogleRequest(servicesManager.operations().get(rawlsBillingProjectOperation.operationId))(Subsystems.GoogleBilling, count)
        }).map { op =>
          rawlsBillingProjectOperation.copy(done = toScalaBool(op.getDone), errorMessage = Option(op.getError).map(error => toErrorMessage(error.getMessage, error.getCode)))
        }
    }
  ...
  override def beginProjectSetup(project: RawlsBillingProject, projectTemplate: ProjectTemplate, groupEmailsByRef: Map[RawlsGroupRef, RawlsGroupEmail]): Future[Try[Seq[RawlsBillingProjectOperationRecord]]] = {
    ...
    toFutureTry(for {
      ...
      bindings <- retryWhen500orGoogleError(count => {
        executeGoogleRequest(cloudResManager.projects().getIamPolicy(projectName.value, null))(Subsystems.GoogleBilling, count).getBindings
      })
      ...
      policy <- retryWhen500orGoogleError(count => {
        ...
        executeGoogleRequest(cloudResManager.projects().setIamPolicy(projectName.value, new SetIamPolicyRequest().setPolicy(updatedPolicy)))(Subsystems.GoogleBilling, count)
      })
      ...
      operations <- Future.sequence(projectTemplate.services.map { service => retryWhen500orGoogleError(count => {
        executeGoogleRequest(serviceManager.services().enable(service, new EnableServiceRequest().setConsumerId(s"project:${projectName.value}")))(Subsystems.GoogleBilling, count)
      }) map { googleOperation =>
  ...
  override def completeProjectSetup(project: RawlsBillingProject): Future[Try[Unit]] = {
    ...
    toFutureTry(for {
      ...
      _ <- retryWhen500orGoogleError(count => {
        ...
        executeGoogleRequest(computeManager.projects().setUsageExportBucket(projectName.value, usageLoc))(Subsystems.GoogleBilling, count)
      })
    } yield {
      // nothing
    })
  ...
  override def deleteProject(projectName: RawlsBillingProjectName): Future[Unit]= {
    ...
    for {
      ...
      _ <- retryWhen500orGoogleError(count => {
        executeGoogleRequest(resMgr.projects().delete(projectNameString))(Subsystems.GoogleBilling, count)
      })
    } yield {
      // nothing
    }
  ...
  def getRawlsUserForCreds(creds: Credential): Future[RawlsUser] = {
    ...
    Future {
      creds.refreshToken()
      val tokenInfo = executeGoogleRequest(oauth2.tokeninfo().setAccessToken(creds.getAccessToken))(Subsystems.GoogleBilling) // TODO
