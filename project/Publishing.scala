...
object Publishing {
  ...
  val publishSettings: Seq[Setting[_]] =
    ...
    Seq(publishTo := Option(artifactoryResolver(false)), credentials += artifactoryCredentials, crossScalaVersions := List("2.11.12", "2.12.7", "2.13.3"), publishConfiguration := publishConfiguration.value.withOverwrite(true))
