 ...
 */
class WorkflowSubmissionSpec(_system: ActorSystem) extends TestKit(_system) with FlatSpecLike with Matchers with TestDriverComponent with BeforeAndAfterAll with Eventually with RawlsTestUtils with MockitoTestUtils with RawlsStatsDTestUtils {
  import driver.api._
  ...
  class TestWorkflowSubmission(
    ...
    val executionServiceCluster: ExecutionServiceCluster = MockShardedExecutionServiceCluster.fromDAO(new HttpExecutionServiceDAO(mockServer.mockServerBaseUrl, mockServer.defaultWorkflowSubmissionTimeout, workbenchMetricBaseName), dataSource)
  ...
  it should "submit a batch of workflows" in withDefaultTestDatabase {
    withStatsD {
      ...
      capturedMetrics should contain(expectedSubsystemRequestMetric(Subsystems.Cromwell, "post", "workflows.v1.batch"))
      capturedMetrics should contain(expectedSubsystemResponseMetric(Subsystems.Cromwell, 201))
      expectedSubsystemTimerMetrics(Subsystems.Cromwell, "post", "workflows.v1.batch").foreach { m =>
  ...
  "WorkflowSubmissionActor" should "submit workflows" in withDefaultTestDatabase {
    withStatsD {
      ...
      val workflowSubmissionActor = system.actorOf(WorkflowSubmissionActor.props(
        slickDataSource,
        new HttpMethodRepoDAO(mockServer.mockServerBaseUrl, "", workbenchMetricBaseName),
  ...
  it should "foo continue submitting workflow on exec svc error" in withDefaultTestDatabase {
    withStatsD {
      val credential: Credential = mockGoogleServicesDAO.getPreparedMockGoogleCredential()
